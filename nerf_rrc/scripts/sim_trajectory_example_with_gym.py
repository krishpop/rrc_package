...
import finger_simulation.tasks.move_cube_on_trajectory as task


def main():
    goal_json = sys.argv[1]
    goal = json.loads(open(goal_json, "r").read())["_goal"]
    initial_pose = task.move_cube.Pose(position=np.array([-0.0023, 0.0013, 0.034]),
            orientation=np.array([3.04629235e-03, 1.34290364e-02,  7.28152931e-01,  6.85276330e-01])
